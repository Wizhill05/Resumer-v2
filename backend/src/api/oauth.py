...
async def oauth_authorize_get(
    ...
    if not current_user:
        ...
        query_dict["backend_url"] = base_url
        encoded_query = urlencode(query_dict)
        frontend_bridge_url = f"/oauth/authorize?{encoded_query}"
        encoded_bridge = quote(frontend_bridge_url, safe="")
...
<head>
    ...
    <style>
        ...
        .client-name {{ color: #ff4e26; font-weight: 600; }}
...
<body>
    <div class="card">
        ...
        <div class="btn-group">
            <a href="{frontend_url}/api/auth/signin/google?callbackUrl={encoded_bridge}" class="social-btn google-btn">
            ...
            <a href="{frontend_url}/api/auth/signin/github?callbackUrl={encoded_bridge}" class="social-btn github-btn">
...
<head>
    ...
    <style>
        ...
        .provider-badge {{ font-size: 11px; font-weight: 600; text-transform: uppercase; background: #ff4e26; color: #fff; padding: 3px 8px; border-radius: 6px; }}
        ...
        .btn-submit {{ width: 100%; background: #ff4e26; color: #fff; font-weight: 700; border: 2px solid #000; box-shadow: 4px 4px 0px #000; padding: 13px; border-radius: 8px; cursor: pointer; font-size: 14px; transition: transform 0.1s, box-shadow 0.1s; }}
        .btn-submit:hover {{ transform: translate(-1px, -1px); box-shadow: 5px 5px 0px #000; }}
        .btn-submit:active {{ transform: translate(2px, 2px); box-shadow: 2px 2px 0px #000; }}
        .btn-cancel {{ display: inline-block; width: 100%; margin-top: 12px; padding: 10px 0; font-size: 13px; color: #71717a; text-decoration: none; transition: color 0.2s; }}
...
<body>
    ...
    response = HTMLResponse(content=consent_html)
    if token_param:
        is_secure = not ("localhost" in base_url or "127.0.0.1" in base_url)
        response.set_cookie(
            key="auth_token",
            value=token_param,
            max_age=3600,
...
async def oauth_authorize_post(
    ...
    if not user and token:
        try:
            ...
            if tok_email:
                u_res = await db.execute(select(User).where(User.email == tok_email))
